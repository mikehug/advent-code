const getIntersectionDistance = (path1, path2) => {
	let intersectionSteps = 0;
	let path1Array = path1.split(',');
	let path2Array = path2.split(',');
	let map = new Map();
	let currentPosition = [0, 0];
	let steps = 0;
	//Iterate through path directions and set the coordinates along path to a map
	for (let i = 0; i < path1Array.length; i++) {
		const path = path1Array[i];
		let distance = parseInt(path.substring(1));
		let direction = path.slice(0, 1);
		switch (direction) {
		//Set coordinates xy as key in map with their sum as value:
		//dir R x++, L x--, U y++, D y--
		//distance number of coordinates to set
		case 'R':
			for (let i = 0; i < distance; i++) {
				currentPosition[0]++;
				steps++;
				if (!map.has(currentPosition)) {
					map.set(currentPosition.toString(), steps);
				}
			}
			break;
		case 'L':
	...
	//reset coordinates
	currentPosition = [0, 0];
	//Iterate through 2nd path directions and check if coordinates along path exist in map
	//IF the coordinate already exist then set it to the value unles value stored there is less 
	steps = 0;
	for (let i = 0; i < path2Array.length; i++) {
		const path = path2Array[i];
		let distance = parseInt(path.substring(1));
		let direction = path.slice(0, 1);
		switch (direction) {
		//dir R x++, L x--, U y++, D y--
		case 'R':
			for (let i = 0; i < distance; i++) {
				currentPosition[0]++;
				steps++;
				if (map.has(currentPosition.toString()) && currentPosition.toString() != '0,0') {
					if (intersectionSteps == 0 || intersectionSteps > map.get(currentPosition.toString()) + steps) {
						intersectionSteps = map.get(currentPosition.toString()) + steps;
					}
				}
			}
			break;
