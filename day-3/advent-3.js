	...
	for (let i = 0; i < path1Array.length; i++) {
		...
		case 'R':
			for (let i = 0; i < distance; i++) {
				currentPosition[0]++;
				value = Math.abs(currentPosition[0]) + Math.abs(currentPosition[1]);
				map.set(currentPosition.toString(), value);
			}
			break;
		case 'L':
			for (let i = 0; i < distance; i++) {
		...
		case 'R':
			for (let i = 0; i < distance; i++) {
				...
				if (map.has(currentPosition.toString()) && currentPosition.toString() != '0,0') {
					value = Math.abs(currentPosition[0]) + Math.abs(currentPosition[1]);
					if (intersectionDistance == 0 || intersectionDistance > value) {
						intersectionDistance = value;
					}
				}
			}
			break;
